...
User Info    
    Get User Info    ${USERNAME}    Karen Anne
...
Get User Info   
    [Arguments]     ${USERNAME}   ${ExpectedName}
    ${response}=    GET    ${BASE_URL}User/GetUserInfoByEmail/${USERNAME}     expected_status=200
    Log To Console   ${response.json().get("name")}
    Compare API And UI Value    ${response.json().get("name")}    ${ExpectedName}
